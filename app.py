...
# Header
st.markdown('<div class="main-header">AI Resume Parser &amp; Candidate Matcher</div>', unsafe_allow_html=True)
        ...
        for idx, res in enumerate(results, 1):
            ...
            with st.container():
                ...
                with col_info:
                    ...
                    # Contact row
                    cnt = cand["contact"]
                    contact_bits = [f"Email: {cnt['email']}", f"Phone: {cnt['phone']}", f"Location: {cnt['location']}"]
                    if cnt.get("github") != "N/A":
                        contact_bits.append(f"GitHub: {cnt['github']}")
                    if cnt.get("linkedin") != "N/A":
                        contact_bits.append(f"LinkedIn: {cnt['linkedin']}")
                ...
                with st.expander(f"View Full Candidate Profile ({cand['name']})"):
                    ...
                    with col_m2:
                        st.write("##### Raw Parsed Resume Text")
...
with tab3:
    ...
    st.write("Export parsed candidate database and search scores for the HR team.")
